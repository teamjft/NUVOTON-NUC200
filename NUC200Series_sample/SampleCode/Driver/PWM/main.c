 ...
 * $Revision: 7 $
 * $Date: 12/08/06 4:29p $ 
 * @brief    NUC200 Series PWM Generator and Capture Timer Driver Sample Code
 *
 * @note
...
{    
    /*In this sample code, user will not use PWMB channel 0~3 PIIR interrupt and CAPIF capture interrupt. 
      Defined following code as 0 for reducing interrupt execution time and code size. */        
#if 0  
// If user want to use captuer functon, PWMB->PBCR must be set to 1 to prevent CFLRIx and CRLRIx is cleared when writing CCR0/CCR2. 
    uint32_t u32PwmIntFlag, u32CapIntFlag0, u32CapIntFlag1;

    /* Handle PWMA Capture function */
    u32CapIntFlag0 = PWMB->CCR0;
...
/*---------------------------------------------------------------------------------------------------------*/
    ...
    /* Enable IP clock */        
    SYSCLK->APBCLK = SYSCLK_APBCLK_UART0_EN_Msk|SYSCLK_APBCLK_PWM01_EN_Msk|SYSCLK_APBCLK_PWM23_EN_Msk;
    /* IP clock source */
    SYSCLK->CLKSEL1 = SYSCLK_CLKSEL1_UART_PLL|SYSCLK_CLKSEL1_PWM01_XTAL|SYSCLK_CLKSEL1_PWM23_XTAL;
...
/*---------------------------------------------------------------------------------------------------------*/
    ...
    /* Configure write 1 to clear CFLRI0~3 and CRLRI0~3 for PWMA Capture fucntion */
    /* Note: It is strongly recommend that set this bit to 1 to prevent CFLRIx and CRLRIx is cleared when writing CCR0/CCR2.*/
    PWMA->PBCR = 1;
...
{   
    ...
    printf("|                                                                        |\n");
    printf("+------------------------------------------------------------------------+\n");                    
    printf("  This sample code will use PWMA channel 0 to drive Buzzer or use PWMA channel 2 to capture\n  the signal from PWMA channel 1.\n");
    printf("  I/O configuration:\n");
    printf("    PWM0(PA.12 PWMA channel 0) <--> Buzzer\n");
    printf("    PWM1(PA.13 PWMA channel 1) <--> PWM2(PA.14 PWMA channel 2)\n\n");
    ...
    {       
        ...
        switch (u8Item)
        {
            ...
            {           
                ...
                printf("\nPWM Timer Waveform Test. Waveform output(PA.12 PWMA channel 0) to Buzzer\n");  
                /* PA.12 PWMA channel 0 generates PWM frequency Do - Si */
                ...
                {                   
                    ...
                    */                                                                 
                        ...
                        _PWM_ENABLE_TIMER_PERIOD_INT(PWMA, PWM_CH0);
                        ...
                        _PWM_DISABLE_TIMER(PWMA, PWM_CH0); 
                        
                        /* Disable PWM Output pin */
                        _PWM_DISABLE_PWM_OUT(PWMA, PWM_CH0);                         
                    }
                }   
                break;
            }  
            ...
            case '2':
            {
                ...
                printf("Use PWMA Channel 2(PA.14) to capture the PWMA Channel 1(PA.13) Waveform\n");       
                ...
                */                                
                ...
                /*Set PWM Timer clock prescaler*/
                _PWM_SET_TIMER_PRESCALE(PWMA,PWM_CH2, 1); // Divided by 2  
                ...
                _PWM_ENABLE_CAP_FALLING_INT(PWMA, PWM_CH2);
                
                /* Enable PWMA NVIC interrupt */
                NVIC_EnableIRQ((IRQn_Type)(PWMA_IRQn)); 

                /* Enable Capture Function for PWMA channel 2 */
                _PWM_ENABLE_CAP_FUNC(PWMA, PWM_CH2);
                ...
                _PWM_ENABLE_CAP_IN(PWMA, PWM_CH2);
                ...
                _PWM_DISABLE_TIMER(PWMA, PWM_CH1);
                             
                /* Disable PWM Output path for PWMA channel 1 */
                _PWM_DISABLE_PWM_OUT(PWMA, PWM_CH1);
    ...
    /* Disable PWMA clock engine */
    SYSCLK->APBCLK&=(~(SYSCLK_APBCLK_PWM01_EN_Msk|SYSCLK_APBCLK_PWM23_EN_Msk));          
    return 0;  
}
