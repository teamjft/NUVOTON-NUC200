 ...
 * $Revision: 3 $
 * $Date: 8/07/12 1:23p $ 
 * @brief    NUC200 Series Mass Storage API
 *
 * @note
...
void UsbCfg(void)
{
    ...
    USBD->STBUFSEG = 248;                         /* Buffer for setup packet */
    _USBD_SET_CFG_EP0(EPT_CFGP);                /* Clear EP0 CTRL IN Ready Flag */
    _USBD_SET_CFG(0, CFG0_SETTING);             /* EP0 ==> control IN end point, address 0 */
    _USBD_SET_EP_BUF(0, USBD_SRAM_BASE + BUF0_SETTING);  /* Buffer for EP0 */
    _USBD_SET_CFG_EP1(EPT_CFGP);                /* Clear EP1 CTRL OUT Ready Flag */
    _USBD_SET_CFG(1, CFG1_SETTING);             /* EP1 ==> control OUT end point, address 0 */
    _USBD_SET_EP_BUF(1, USBD_SRAM_BASE + BUF1_SETTING);    /* Buffer for EP1 */
    
    /* Init Bulk IN end point */
    _USBD_SET_CFG_EP2(EPT_CFGP);                /* Clear EP2 BULK IN Ready Flag */
    _USBD_SET_CFG(2, CFG2_SETTING);             /* EP2 ==> Bulk IN end point, address 2 */
    _USBD_SET_EP_BUF(2, USBD_SRAM_BASE + BUF_BULK1);     /* Buffer for EP2 (Bulk IN) */  
  
    /* Init Bulk OUT end point */
    _USBD_SET_CFG_EP3(EPT_CFGP);                /* Clear EP3 BULK OUT Ready Flag */
    _USBD_SET_CFG(3, CFG3_SETTING);             /* EP3 ==> Bulk OUT end point, address 3 */
    _USBD_SET_EP_BUF(3, USBD_SRAM_BASE + BUF_BULK0);   /* Buffer for EP3 (Bulk OUT) */
...
int32_t UsbStdReq(void)
{
    ...
    switch (tmp[1])
  {
    case GET_DESCRIPTOR:
    {
      ...
      _USBD_SET_CFG(0, DATA1 (CFG0_SETTING));
      _USBD_TRIG_EP(0, len);
...
int32_t UsbClassReq(void)
{
  ...
  switch(tmp[1])
    ...
    case BULK_ONLY_MASS_STORAGE_RESET:
    {
      ...
      _USBD_SET_CFG(0, DATA1 (CFG0_SETTING));
      _USBD_TRIG_EP(0, 0);
      _USBD_SET_CFG(2, DATA0 (CFG2_SETTING));
    ...
    case GET_MAX_LUN:
    {
      ...
      _USBD_SET_CFG(0, DATA1(CFG0_SETTING));
      _USBD_SET_CFG(3, DATA0(CFG3_SETTING));
      _USBD_TRIG_EP(0, 1);
...
int32_t UsbSetup(void)
{
  ...
  _USBD_SET_CFG_EP0(CFGP_CLRRDY); 
  _USBD_SET_CFG_EP1(CFGP_CLRRDY);
...
void UsbRead(void)
{
  ...
  if (*((__IO uint32_t *)(&USBD->EP[2].BUFSEG)) == BUF_BULK1)
    _USBD_SET_EP_BUF(2, USBD_SRAM_BASE + BUF_BULK0);
  else
    _USBD_SET_EP_BUF(2, USBD_SRAM_BASE + BUF_BULK1);
    
  /* Trigger to send out the data packet */ 
  _USBD_TRIG_EP(2, g_u8Size);
...
void UsbBulkInAck(void)
{
  ...
  if (g_u8BulkState == BULK_CSW)
  {
    ...
    _USBD_SET_EP_BUF(3, USBD_SRAM_BASE + BUF_BULK0);
    _USBD_TRIG_EP(3, 31);
  }
...
void UsbWrite(void)
{
  ...
  if (g_u32Length > MAX_PACKET_SIZE)
  {
    ...
    {
      _USBD_SET_EP_BUF(3, USBD_SRAM_BASE + BUF_BULK1);
      _USBD_TRIG_EP(3, MAX_PACKET_SIZE);
    ...
    switch (g_sCBW.u8OPCode)
    {
      ...
      case UFI_READ_10:
      {
        ...
        if(g_u32BytesInStorageBuf > 0)
        {
          ...
          _USBD_SET_EP_BUF(2, USB_SRAM_BASE + BUF_BULK0);
          UsbRead();
#endif
        }
        else
...
{
  uint32_t u32STS = _USBD_GET_EPSTS();
  ...
  { 
    ...
    { 
      ...
      _USBD_SET_CFGP(0, (CFGP_SSTALL | CFGP_CLRRDY));
      _USBD_SET_CFGP(1, (CFGP_SSTALL | CFGP_CLRRDY));
    }
  }
  else if(u32INTSTS & INTSTS_EPTF0)
  { 
      /* Ctrl In */
    _USBD_SET_EVENT_FLAG(INTSTS_EPTF0);
    UsbInAck();
  }
  else if(u32INTSTS & INTSTS_EPTF1)
  { 
      /* Ctrl Out */
    _USBD_SET_EVENT_FLAG(INTSTS_EPTF1);
    UsbOutAck();
  }
  else if(u32INTSTS & INTSTS_EPTF2)
  { 
      /* Bulk In */
    _USBD_SET_EVENT_FLAG(INTSTS_EPTF2);
  ...
  { 
      /* Bulk Out */
    _USBD_SET_EVENT_FLAG(INTSTS_EPTF3);
    ...
    else
    {
      ...
      { 
          /* stall */
        g_bCBWInvalid = TRUE;
        _USBD_SET_CFGP(2, (CFGP_SSTALL | CFGP_CLRRDY));
        _USBD_SET_CFGP(3, (CFGP_SSTALL | CFGP_CLRRDY));
      }
    }
  }
  else
  { /* Unknow event */
    _USBD_SET_EVENT_FLAG(INTSTS_USB); 
  }
}
...
void UsbIsr(void)
{
    uint32_t u32INTSTS = _USBD_GET_EVENT_FLAG();
...
void udcInit(void)
{
    ...
    SYSCLK->CLKDIV = (SYSCLK->CLKDIV & (~SYSCLK_CLKDIV_USB_N_Msk)) | SYSCLK_CLKDIV_USB(1);
