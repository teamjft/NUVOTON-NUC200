 ...
 * $Revision: 3 $
 * $Date: 8/07/12 2:01p $ 
 * @brief    NUC200 Series USB VOCM System API
 *
 * @note
...
S_USBD_EVENT_PROCESS g_sBusOps[6] = 
{
    ...
    {USBD_BusResetCallback, &gsVcomDevice},     /* Bus reset event callback     */
    {NULL, NULL},                               /* Bus suspend event callback   */
    {NULL, NULL},                               /* Bus resume event callback    */
    {USBD_CtrlSetupAck, &gsVcomDevice},         /* Setup event callback         */
};

/* USB event call back functions */
S_USBD_EVENT_PROCESS g_sUsbOps[12] = 
{
    {USBD_CtrlDataInAck, &gsVcomDevice},
    {USBD_CtrlDataOutAck, &gsVcomDevice},
...
S_USBD_CTRL_CALLBACK_ENTRY g_asCtrlCallbackEntry[] = {
    ...
    {REQ_STANDARD, SET_ADDRESS, USBD_CtrlSetupSetAddress, USBD_CtrlDataInSetAddress, 0, &gsVcomDevice},
    {REQ_STANDARD, CLEAR_FEATURE, USBD_CtrlSetupClearSetFeature, 0, 0, &gsVcomDevice},
    {REQ_STANDARD, SET_FEATURE, USBD_CtrlSetupClearSetFeature, 0, 0, &gsVcomDevice},
    {REQ_STANDARD, GET_CONFIGURATION, USBD_CtrlSetupGetConfiguration, 0, 0, &gsVcomDevice},
    {REQ_STANDARD, GET_STATUS, USBD_CtrlSetupGetStatus, 0, 0, &gsVcomDevice},
    {REQ_STANDARD, GET_INTERFACE, USBD_CtrlSetupGetInterface, 0, 0, &gsVcomDevice},
    {REQ_STANDARD, SET_INTERFACE, USBD_CtrlSetupSetInterface, 0, 0, &gsVcomDevice},
...
void VCOM_CtrlDataInGetDescriptor(void* pVoid)
{
    ...
    if (psDevice->au8Setup[0] & 0x80)
    {
        if(gu16TransferLen)
        {
            ...
            {       
                USBD_DataIn(0, gau8ConfigDescriptor + gu16TransferIndex, gu16PacketLen);
                gu16TransferLen -= gu16PacketLen;
                gu16TransferIndex += gu16PacketLen;
            }
            else
            {
                USBD_DataIn(0, gau8ConfigDescriptor + gu16TransferIndex, gu16TransferLen);
                gu16TransferLen = 0;
            }
            
            if (gu16TransferLen == 0)
                _USBD_TRIG_EP(1, 0x00); //Ctl data out
        }
        else
        {
...
void VCOM_CtrlSetupGetDescriptor(void* pVoid)
{
    ...
    S_USBD_DEVICE *psDevice = (S_USBD_DEVICE *)psVcomDevice->device;
    ...
    switch (psDevice->au8Setup[3])
    {
        ...
        case DESC_DEVICE:
        {
            ...
            USBD_DataIn(0, psVcomDevice->au8DeviceDescriptor, u16Len);
            break;
        }
        ...
        case DESC_CONFIG:
        {
            ...
            if(gu16TransferLen > gu16PacketLen)
            {
                USBD_DataIn(0, psVcomDevice->au8ConfigDescriptor + gu16TransferIndex, gu16PacketLen);
                gu16TransferLen -= gu16PacketLen;
                gu16TransferIndex += gu16PacketLen;
            }
            else
            {
                USBD_DataIn(0, psVcomDevice->au8ConfigDescriptor + gu16TransferIndex, gu16TransferLen);
                gu16TransferLen = 0;
            }
            break;
        }
        ...
        case DESC_STRING:
        {
            ...
            if (psDevice->au8Setup[2] == 0)
            {
                ...
                USBD_DataIn(0, gau8StringLang, u16Len);
            }
            else
            {
                /* Get String Descriptor */
                switch (psDevice->au8Setup[2])
                {
                    case 1:
                        ...
                        USBD_DataIn(0, (const uint8_t *)psVcomDevice->sProductStringDescriptor, u16Len);
                        break;
                    case 2:
                    ...
                    case 3:
                        ...
                        USBD_DataIn(0, gau8StringSerial, u16Len);
                        break;
                    default:
...
{
    S_USBD_DEVICE *psDevice = (S_USBD_DEVICE *)((S_VCOM_DEVICE *)pVoid)->device;
    DBG_PRINTF("Set line coding\n");
    psDevice->abData0[0] = FALSE;
    USBD_DataOutTrigger(0, 7);
}
...
void VCOM_GetLineCoding(void* pVoid)
{
    ...
    USBD_DataIn(0, (void *)&gLineCoding, 7);
}
...
void VCOM_CtrlSetupSetConfiguration(void* pVoid)
{
    ...
    else if(bIsDeviceConfigure)
    {
        ...
        USBD_DataOutTrigger(BULK_OUT_EP_NUM, 64);
        USBD_DataIn(BULK_IN_EP_NUM, 0, 0);
    }
    else
    {
        // Not support. Reply STALL.
...
int32_t VCOM_Open(void)
{
    ...
    psVcomDevice->device = (void*)USBD_InstallClassDevice(&sVcomClass);
    ...
    USBD_InstallCtrlHandler(psVcomDevice->device, g_asCtrlCallbackEntry, 
