 ...
 * $Revision: 3 $
 * $Date: 12/08/06 5:57p $ 
 * @brief    NUC200 Series USB HID System API
 *
 * @note
...
S_USBD_EVENT_PROCESS g_sBusOps[6] = 
{
	...
	{USBD_BusSuspendCallback, &g_HID_sDevice},/* bus suspend event callback */
	{USBD_BusResumeCallback, &g_HID_sDevice},                               /* bus resume event callback */
...
void HID_CtrlSetupGetProtocol(void * pVoid)
{
	...
	USBD_DataIn(0, (const uint8_t *)&psDevice->isReportProtocol, 1);
}
