 ...
 * $Revision: 3 $
 * $Date: 8/07/12 11:34a $ 
 * @brief    NUC200 Series UAC HID System API
 *
 * @note
...
S_USBD_EVENT_PROCESS g_sBusOps[6] = 
{
	{NULL, NULL},//attach
	{NULL, NULL},//detach
	{USBD_BusResetCallback, &g_UAC_sDevice},//bus reset
	{NULL, NULL},//bus suspend
	{NULL, NULL},//bus resume
	{USBD_CtrlSetupAck, &g_UAC_sDevice},//setup
};

/* USB event call back function	*/
S_USBD_EVENT_PROCESS g_sUsbOps[12] = 
{
	{USBD_CtrlDataInAck, &g_UAC_sDevice},
	{USBD_CtrlDataOutAck, &g_UAC_sDevice},//ctrl pipe0
...
{  /* request type,command     ,setup ack handler         , in ack handler      ,out ack handler,  parameter */
	{REQ_STANDARD, SET_ADDRESS, USBD_CtrlSetupSetAddress, USBD_CtrlDataInSetAddress, NULL, &g_UAC_sDevice},
	{REQ_STANDARD, CLEAR_FEATURE, USBD_CtrlSetupClearSetFeature, NULL, NULL, &g_UAC_sDevice},
	{REQ_STANDARD, SET_FEATURE, USBD_CtrlSetupClearSetFeature, NULL, NULL, &g_UAC_sDevice},
	{REQ_STANDARD, GET_CONFIGURATION, USBD_CtrlSetupGetConfiguration, NULL, NULL, &g_UAC_sDevice},
	{REQ_STANDARD, GET_STATUS, USBD_CtrlSetupGetStatus, NULL, NULL, &g_UAC_sDevice},
	...
	{REQ_STANDARD, SET_CONFIGURATION, USBD_CtrlSetupSetConfiguration, NULL, NULL, &g_UAC_sDevice},
...
S_USBD_CLASS sUacUsbClass = 
{
	(void *)&g_UAC_sDevice, 
	(PFN_USBD_CALLBACK)UAC_Reset,
	UAC_IsCfgId
};
...
void UAC_CtrlSetupGetInterface(void * pVoid)
{
	S_UAC_DEVICE *psDevice = (S_UAC_DEVICE *) pVoid;
  	...
  	USBD_DataIn(0, &psDevice->u8AlternativeSetting, 1);
}
...
void UAC_CtrlSetupSetInterface(void * pVoid)
{
    S_USBD_DEVICE *psUSBDevice = &gsUsbDevice;
    S_UAC_DEVICE *psDevice = (S_UAC_DEVICE *)pVoid;
    ...
    if (psUSBDevice->au8Setup[4] == 1)
    {
    	...
    	if (psDevice->u8AlternativeSetting == 1)
	    {
            ...
            USBD_DataIn(ISO_IN_EP_NUM, NULL, 0);
	...
	else if (psUSBDevice->au8Setup[4] == 2)
	{
    	...
    	if (psDevice->u8AlternativeSetting == 1)
	    {
     	   	USBD_DataOutTrigger(ISO_OUT_EP_NUM, psDevice->u8IsoOutPacketSize);
...
void UAC_EpNum1InEvent(void * pVoid)
{
	...
	S_UAC_DEVICE *psDevice = (S_UAC_DEVICE *) pVoid;
	S_USBD_DEVICE *psUsbDevice;
	int32_t u32EpId;	
	
	psUsbDevice = (S_USBD_DEVICE *)psDevice->device;
	
	u32EpId = USBD_GetEpId(ISO_IN_EP_NUM | 0x80);
	...
	if (u32PacketType == EPSTS_ISO)
	{
        ...
        {         
            u32EpId = USBD_GetEpIdentity(ISO_IN_EP_NUM, EP_INPUT);
            _USBD_TRIG_EP(u32EpId, 0);
        }
    }
}
...
void UAC_EpNum2OutEvent(void * pVoid)
{
    ...
    pcmBuf = USBD_GetOutData(ISO_OUT_EP_NUM, &tmp);
    pfnUacGetPlayData((int16_t *)pcmBuf, (int16_t)tmp);
    u32EpId = USBD_GetEpIdentity(ISO_OUT_EP_NUM, EP_OUTPUT);
    
    _USBD_SET_EP_BUF(u32EpId, (uint32_t)(pcmBuf));
    _USBD_TRIG_EP(u32EpId, ISO_OUT_MXPLD);
...
{		
	...
	g_UAC_sDevice.device = (void *)USBD_InstallClassDevice(&sUacUsbClass);
	...
	USBD_InstallCtrlHandler(g_UAC_sDevice.device, g_asCtrlCallbackEntry, 
					sizeof(g_asCtrlCallbackEntry) / sizeof(g_asCtrlCallbackEntry[0]));

    USBD_EnableSelfPower();
...
void UAC_Reset(void * *pVoid)
{
    ...
    u32EpId = USBD_GetEpIdentity(ISO_IN_EP_NUM, EP_INPUT);
  	_USBD_SET_CFG(u32EpId, CFG_ISO_IN);
	_USBD_SET_CFGP(u32EpId, CFGP_CLRRDY);
    _USBD_SET_EP_BUF(u32EpId, (uint32_t)psUsbDevice->sEpCrl[u32EpId].u8SramBuffer);
    
    /* Reset ISO OUT endpoint setting */    
    u32EpId = USBD_GetEpIdentity(ISO_OUT_EP_NUM, EP_OUTPUT);
	_USBD_SET_CFG(u32EpId, CFG_ISO_OUT);
	_USBD_SET_CFGP(u32EpId, CFGP_CLRRDY);  
}
...
void UAC_SendOneAudioPacket(uint8_t *pbyAudioData, uint8_t u8AudioSize)
{
    if (u8AudioSize > 0)
    {
        USBD_DataIn(ISO_IN_EP_NUM, (uint8_t *)pbyAudioData, u8AudioSize);
    }
    else
    {
       _USBD_TRIG_EP(USBD_GetEpIdentity(ISO_IN_EP_NUM, EP_INPUT), 0);  
    }      
}
...
{
	S_UAC_DEVICE *psDevice = (S_UAC_DEVICE *) pVoid;
	S_USBD_DEVICE *pInfraDevice = (S_USBD_DEVICE *)psDevice->device;
	uint16_t u16Len;
...
{
  	S_UAC_DEVICE *psDevice = (S_UAC_DEVICE *) pVoid;
  	S_USBD_DEVICE *psUsbDevice = (S_USBD_DEVICE *)psDevice->device;
  	...
  	if (REC_FEATURE_UNITID == psUsbDevice->au8Setup[5])
  	{
        ...
        {
            case MUTE_CONTROL:
               _USBD_TRIG_EP(1,psUsbDevice->au8Setup[6]); 
               break;
            case VOLUME_CONTROL:
               _USBD_TRIG_EP(1,psUsbDevice->au8Setup[6]); 
               break;
